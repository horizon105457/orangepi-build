...
InstallDroneCAN() {
	...
	pip3 install --no-cache-dir dronecan
...
overlays=opicm5-cc-usb-net opicm5-cc-cam1 can1-m1 uart1-m1 uart6-m1 uart4-m2 opicm5-cc-pwm-io spi1-m2-cs0-spidev
...
Main() {
	...
	# SKIP: boot-test only — RT tuning, DNS, autologin cleanup not needed
	...
	case $RELEASE in
		...
		noble)
			...
			# Wi-Fi: handled by NM connection file in overlay
			# (etc/NetworkManager/system-connections/Master.nmconnection)
			# SKIP: boot-test only — no extra packages needed
			# echo "[main] Noble: Installing mDNS"
			# InstallMDNS
			# SKIP: heavy packages — experimental/6.18-build boot test only
			# echo "[main] Noble: Installing ROS2 Jazzy"
			# InstallROS2 jazzy
			# echo "[main] Noble: Installing OpenCV"
			# InstallOpenCV
			# echo "[main] Noble: Installing DroneCAN tooling"
			# InstallDroneCAN
			# SKIP: boot-test only — NPU/render not needed for kernel validation
			# echo "[main] Noble: Ensuring render group access for DRM NPU path"
			# EnsureRenderGroupAccess
			# echo "[main] Noble: Installing acceleration healthcheck"
			# InstallAccelHealthcheck || true
			# SKIP: remote desktop not needed for boot test
			# if [[ "${BUILD_DESKTOP}" == "yes" ]]; then
	...
	# SKIP: boot-test only — doc sync and overlay hints not needed
	# echo "[main] Step 3: Sync OPiBot files to user Documents"
	# InstallRuntimeDeploymentGuide
	# echo "[main] Step 4: Inject CM5-CC overlay hints"
	# InjectCm5CcOverlayHints
